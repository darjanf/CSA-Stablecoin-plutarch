...
collateralValidatorT = phoistAcyclic $ plam $ \datum redeemer ctx -> unTermCont $ do
    ...
    infoFields                      <- pletFieldsC @'["signatories", "mint", "withdraw"] ctxFields.txInfo
    ...
    pure $ pif ( pmatch redeemer $ \case 
            PRedeem -> pand'List [checkSignedByCollOwner, checkMintedItems, checkStablecoinAmount]
            PLiquidate -> pand'List [checkStablecoinAmount]
        )
        (popaque (pconstant ()))
        perror
